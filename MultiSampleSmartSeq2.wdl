...
workflow MultiSampleSmartSeq2 {
    ...
    call MutationCall.GenomicsDBImport as GenomicsDBImport {
        input:
            workflowOutputDir = workflowOutputDir + "variants/",
            collectionName = collectionName,
            gvcfFiles = HaplotypeCallerGvcf.outputGvcfString
    }

    call MutationCall.GenotypeGvcfs as GenotypeGvcfs {
        input:
            passToGenotypeGvcfs = GenomicsDBImport.passToGenotypeGvcfs,
            workflowOutputDir = workflowOutputDir + "variants/",
            collectionName = collectionName
    }
}
...
task cleanUpFiles {
    input {
        Array[File] fileQueue
    }

    command <<<
        set -e
        
        declare -a files=(~{sep=' ' fileQueue})
    >>>
}
